...
#ifndef LogScannerBASE_H_INCLUDED
#define LogScannerBASE_H_INCLUDED
...
private:
    ...
    size_t (LogScannerBase::*d_get)() = &LogScannerBase::getInput;
...
protected:
    ...
    int     const (*d_dfaBase_)[58];

    static int     const s_dfa_[][58];
    static int     const (*s_dfaBase_[])[58];
...
public:
    LogScannerBase(LogScannerBase const &other)             = delete;
...
protected:
    LogScannerBase(std::istream &in, std::ostream &out, bool keepCwd = true);
    LogScannerBase(std::string const &infilename, std::string const &outfilename, bool keepCwd = true);
    ~LogScannerBase();
...
inline LogScannerBase::~LogScannerBase()
{
    d_input->close();
}

template <typename ReturnType, typename ArgType>
inline ReturnType constexpr LogScannerBase::as(ArgType value)
{
    return static_cast<ReturnType>(value);
}

// $insert startcondimpl
inline LogScannerBase::StartCondition_ constexpr LogScannerBase::SC(int sc)
{
    return as<StartCondition_>(sc);
}

inline int constexpr LogScannerBase::SC(StartCondition_ sc)
{
    return as<int>(sc);
}

inline LogScannerBase::StartCondition_ LogScannerBase::startCondition() const
{
    return SC(d_startCondition);
}

inline void LogScannerBase::begin(StartCondition_ startCondition)
{
    // d_state is reset to 0 by reset_()
    d_dfaBase_ = s_dfaBase_[d_startCondition = SC(startCondition)];
}

inline bool LogScannerBase::knownFinalState()
{
    return (d_atBOL && available(d_final.bol.rule)) ||
           available(d_final.std.rule);
}

inline bool constexpr LogScannerBase::available(size_t value)
{   
    return value != std::numeric_limits<size_t>::max();
}

inline std::ostream &LogScannerBase::out()
{
    return *d_out;
}

inline void LogScannerBase::push(size_t ch)
{
    d_input->reRead(ch);
}

inline void LogScannerBase::push(std::string const &str)
{
    d_input->reRead(str, 0);
}


inline std::vector<LogScannerBase::StreamStruct> const &LogScannerBase::streamStack() const
{
...
inline void LogScannerBase::setFilename(std::string const &name)
{
    d_filename = name;
}

inline void LogScannerBase::setMatched(std::string const &text)
{
    d_matched = text;
}

inline std::string const &LogScannerBase::matched() const
{
    return d_matched;
}

inline std::string const &LogScannerBase::cwd() const
{
    return d_cwd;
}

inline std::string const &LogScannerBase::filename() const
{
    return d_filename;
}

inline void LogScannerBase::echo() const
{
    *d_out << d_matched;
}

inline size_t LogScannerBase::length() const
{
    return d_matched.size();
}

inline void LogScannerBase::leave(int retValue) const
{
    throw as<Leave_>(retValue);
}

inline size_t LogScannerBase::lineNr() const
{
    return d_input->lineNr();
}

inline void LogScannerBase::more()
{
    d_more = true;
}

inline size_t LogScannerBase::state_() const
{
    return d_state;
}

inline size_t LogScannerBase::get_()
{
    return (this->*d_get)();
}

inline size_t LogScannerBase::getInput()
{
    return d_input->get();
}

inline bool LogScannerBase::return_()
{
    return d_return;
}

inline void LogScannerBase::noReturn_()
{
    d_return = false;
}


#endif //  LogScannerBASE_H_INCLUDED
