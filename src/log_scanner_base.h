...
#ifndef log_scannerBASE_H_INCLUDED
#define log_scannerBASE_H_INCLUDED
...
private:
    ...
    size_t (log_scannerBase::*d_get)() = &log_scannerBase::getInput;
...
public:
    log_scannerBase(log_scannerBase const &other)             = delete;
...
protected:
    log_scannerBase(std::istream &in, std::ostream &out, bool keepCwd = true);
    log_scannerBase(std::string const &infilename, std::string const &outfilename, bool keepCwd = true);
    ~log_scannerBase();
...
inline size_t log_scannerBase::Input::lineNr() const
{
    return d_lineNr;
}
inline size_t log_scannerBase::Input::nPending() const
{
    return d_deque.size();
}
inline void log_scannerBase::Input::setPending(size_t size)
{
    d_deque.erase(d_deque.begin(), d_deque.end() - size);
}
inline void log_scannerBase::Input::close()
{
    delete d_in;
...
inline log_scannerBase::~log_scannerBase()
{
    d_input->close();
}

template <typename ReturnType, typename ArgType>
inline ReturnType constexpr log_scannerBase::as(ArgType value)
{
    return static_cast<ReturnType>(value);
}

// $insert startcondimpl
inline log_scannerBase::StartCondition_ constexpr log_scannerBase::SC(int sc)
{
    return as<StartCondition_>(sc);
}

inline int constexpr log_scannerBase::SC(StartCondition_ sc)
{
    return as<int>(sc);
}

inline log_scannerBase::StartCondition_ log_scannerBase::startCondition() const
{
    return SC(d_startCondition);
}

inline void log_scannerBase::begin(StartCondition_ startCondition)
{
    // d_state is reset to 0 by reset_()
    d_dfaBase_ = s_dfaBase_[d_startCondition = SC(startCondition)];
}

inline bool log_scannerBase::knownFinalState()
{
    return (d_atBOL && available(d_final.bol.rule)) ||
           available(d_final.std.rule);
}

inline bool constexpr log_scannerBase::available(size_t value)
{   
    return value != std::numeric_limits<size_t>::max();
}

inline std::ostream &log_scannerBase::out()
{
    return *d_out;
}

inline void log_scannerBase::push(size_t ch)
{
    d_input->reRead(ch);
}

inline void log_scannerBase::push(std::string const &str)
{
    d_input->reRead(str, 0);
}


inline std::vector<log_scannerBase::StreamStruct> const &log_scannerBase::streamStack() const
{
...
inline void log_scannerBase::setFilename(std::string const &name)
{
    d_filename = name;
}

inline void log_scannerBase::setMatched(std::string const &text)
{
    d_matched = text;
}

inline std::string const &log_scannerBase::matched() const
{
    return d_matched;
}

inline std::string const &log_scannerBase::cwd() const
{
    return d_cwd;
}

inline std::string const &log_scannerBase::filename() const
{
    return d_filename;
}

inline void log_scannerBase::echo() const
{
    *d_out << d_matched;
}

inline size_t log_scannerBase::length() const
{
    return d_matched.size();
}

inline void log_scannerBase::leave(int retValue) const
{
    throw as<Leave_>(retValue);
}

inline size_t log_scannerBase::lineNr() const
{
    return d_input->lineNr();
}

inline void log_scannerBase::more()
{
    d_more = true;
}

inline size_t log_scannerBase::state_() const
{
    return d_state;
}

inline size_t log_scannerBase::get_()
{
    return (this->*d_get)();
}

inline size_t log_scannerBase::getInput()
{
    return d_input->get();
}

inline bool log_scannerBase::return_()
{
    return d_return;
}

inline void log_scannerBase::noReturn_()
{
    d_return = false;
}


#endif //  log_scannerBASE_H_INCLUDED
