...
#ifndef HeaderScannerBASE_H_INCLUDED
#define HeaderScannerBASE_H_INCLUDED
...
private:
    ...
    size_t (HeaderScannerBase::*d_get)() = &HeaderScannerBase::getInput;
...
public:
    HeaderScannerBase(HeaderScannerBase const &other)             = delete;
...
protected:
    HeaderScannerBase(std::istream &in, std::ostream &out, bool keepCwd = true);
    HeaderScannerBase(std::string const &infilename, std::string const &outfilename, bool keepCwd = true);
    ~HeaderScannerBase();
...
inline HeaderScannerBase::~HeaderScannerBase()
{
    d_input->close();
}

template <typename ReturnType, typename ArgType>
inline ReturnType constexpr HeaderScannerBase::as(ArgType value)
{
    return static_cast<ReturnType>(value);
}

// $insert startcondimpl
inline HeaderScannerBase::StartCondition_ constexpr HeaderScannerBase::SC(int sc)
{
    return as<StartCondition_>(sc);
}

inline int constexpr HeaderScannerBase::SC(StartCondition_ sc)
{
    return as<int>(sc);
}

inline HeaderScannerBase::StartCondition_ HeaderScannerBase::startCondition() const
{
    return SC(d_startCondition);
}

inline void HeaderScannerBase::begin(StartCondition_ startCondition)
{
    // d_state is reset to 0 by reset_()
    d_dfaBase_ = s_dfaBase_[d_startCondition = SC(startCondition)];
}

inline bool HeaderScannerBase::knownFinalState()
{
    return (d_atBOL && available(d_final.bol.rule)) ||
           available(d_final.std.rule);
}

inline bool constexpr HeaderScannerBase::available(size_t value)
{   
    return value != std::numeric_limits<size_t>::max();
}

inline std::ostream &HeaderScannerBase::out()
{
    return *d_out;
}

inline void HeaderScannerBase::push(size_t ch)
{
    d_input->reRead(ch);
}

inline void HeaderScannerBase::push(std::string const &str)
{
    d_input->reRead(str, 0);
}


inline std::vector<HeaderScannerBase::StreamStruct> const &HeaderScannerBase::streamStack() const
{
...
inline void HeaderScannerBase::setFilename(std::string const &name)
{
    d_filename = name;
}

inline void HeaderScannerBase::setMatched(std::string const &text)
{
    d_matched = text;
}

inline std::string const &HeaderScannerBase::matched() const
{
    return d_matched;
}

inline std::string const &HeaderScannerBase::cwd() const
{
    return d_cwd;
}

inline std::string const &HeaderScannerBase::filename() const
{
    return d_filename;
}

inline void HeaderScannerBase::echo() const
{
    *d_out << d_matched;
}

inline size_t HeaderScannerBase::length() const
{
    return d_matched.size();
}

inline void HeaderScannerBase::leave(int retValue) const
{
    throw as<Leave_>(retValue);
}

inline size_t HeaderScannerBase::lineNr() const
{
    return d_input->lineNr();
}

inline void HeaderScannerBase::more()
{
    d_more = true;
}

inline size_t HeaderScannerBase::state_() const
{
    return d_state;
}

inline size_t HeaderScannerBase::get_()
{
    return (this->*d_get)();
}

inline size_t HeaderScannerBase::getInput()
{
    return d_input->get();
}

inline bool HeaderScannerBase::return_()
{
    return d_return;
}

inline void HeaderScannerBase::noReturn_()
{
    d_return = false;
}


#endif //  HeaderScannerBASE_H_INCLUDED
