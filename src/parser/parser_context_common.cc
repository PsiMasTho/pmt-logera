...
{
    auto const var_itr = find_if(hd.vars.begin(), hd.vars.end(), [&var_name](variable_data const& var) { return var.name == var_name; });
...
{
    auto const attr_itr =
        find_if(hd.attrs.begin(), hd.attrs.end(), [&attr_name](attribute_data const& attr) { return attr.name == attr_name; });
...
{
    auto const var_itr = lower_bound(hd.vars.begin(), hd.vars.end(), var_name, [](variable_data const& var, std::string const& var_name) {
...
{
    auto const attr_itr =
        lower_bound(hd.attrs.begin(), hd.attrs.end(), attr_name, [](attribute_data const& attr, std::string const& attr_name) {
