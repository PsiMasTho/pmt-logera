    ...
    d_optionToValueMap{}
{
    ...
    for (size_t i = 1; argv[i]; ++i)
    {
		...
		if (ArgType argtype = _argType(argv[i]); argtype != ArgType::VALUE)
		{
            ...
            for (;j < optionCount; ++j)
            {
                string const optStr = argtype == LONG_OPT ? options[j].getName() : _toStr(options[j].getCh());
		...
		else // argv[i] must be a value
        {
            if (target == magicOpt && i == 1)
                ...
                _appendToString(rawOption(target).second,  argv[i] , rawOption(target).second.empty() ? "" : " ");
        }
    }
...
string* Args::createStringForOpt(_Opt const& opt, bool wasSpecified)
{
    ...
    for (auto const& key : keys)
    {
        visit([&](auto&& k)
        {
            success += d_optionToValueMap.emplace(k, make_pair(wasSpecified, str)).second;}, key);
        }
...
pair<bool, string&> Args::rawOption(key_t const& option) const
{
    ...
    if (itr == d_optionToValueMap.end())
        throw invalid_argument("Nonexistant option queried: "s + visit([](auto&& k){return _toStr(k);}, option));
