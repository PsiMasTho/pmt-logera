...
std::string const& Args::option(bool* wasSpecified, char option) const
{
    if (!d_vals.exists(mapCh(option))) // option not specified
    {
        ...
        *wasSpecified = true;
        return d_vals.get(
            mapCh(option)); // is specified, return string (may be empty)
    }
}

void Args::setTypes(char const* optNTBS)
{
    static regex const optRegex(
        R"([a-z]\:{0,2})", regex::ECMAScript | regex::optimize | regex::icase);
    ...
    */
    ...
    for (int i = 1; argv[i]; ++i) {
        if (argv[i][0] == '-') // then opt
            ...
            if (optValPairs.empty())
                ...
                string const delim
                    = optValPairs.back().second.empty() ? string {} : " "s;
    ...
    for (auto [opt, val] : optValPairs) {
        if (d_types.exists(mapCh(opt))) {
            ...
            if (val.empty()
                && (optType == ValType::WITHOUT
                    || optType == ValType::OPTIONAL))
                d_vals.set(mapCh(opt), val);
            else if (!val.empty()
                && (optType == ValType::OPTIONAL
                    || optType == ValType::REQUIRED))
                d_vals.set(mapCh(opt), val);
            else if (val.empty() && optType == ValType::REQUIRED)
                throw invalid_argument(
                    "No value specified for argument: "s + '-' + opt);
            else if (!val.empty() && optType == ValType::WITHOUT)
                throw invalid_argument(
                    "Value specified for argument: "s + '-' + opt);
        } else
