...
decltype(Options::d_headerFile) Options::setHeaderFile(Args const& args)
{
    ...
    if (specified_m) {
        ...
        auto itr = find_if(begin(pathVec), end(pathVec),
            [](auto pth) { return pth.extension() == ".lh"; });
        ...
        auto itr = find_if(itrBeg, filesystem::directory_iterator {},
            [](auto entry) { return entry.path().extension() == ".lh"; });
...
decltype(Options::d_logFiles) Options::setLogFiles(Args const& args)
{
    ...
    if (specified_m) {
        ...
        ret.erase(remove_if(begin(ret), end(ret),
            [](auto pth) { return pth.extension() != ".txt"; }));
    } else if (specified_d) {
        filesystem::directory_iterator itrBeg(cvtFunc<filesystem::path>(str_d));

        for (auto itr = itrBeg; itr != filesystem::directory_iterator {}; ++itr)
            if (itr->path().extension() == ".txt") ret.push_back(itr->path());
    }

    if (ret.empty()) throw invalid_argument("No log files provided");
