...
class lexed_file
{
    struct token
    {
        ...
        underlying_token_t type;   // token type (enumeration from tokens.h)
    };
...
public:
    ...
    auto get_token_type_at(uint32_t idx) const -> underlying_token_t;
    auto get_line_nr_at(uint32_t idx) const -> uint32_t;

    void push_token(underlying_token_t tok, uint32_t offset, uint16_t len);
};
