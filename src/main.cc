...
try {
    ...
    if (opts.verbose()) opts.verbosePrint(cerr);

    // parse header
    unique_ptr<HeaderData> headerData = HeaderParser(opts.headerFile()).gen();

    // syntax error encountered
    if (headerData == nullptr) return FAIL;
    ...
    for (auto const& pth : opts.logFiles()) {
        ...
        // syntax error encountered
        if (logData == nullptr) return FAIL;

        parsedData.push_back(move(logData));
    }

    // sort the output by date
    stable_sort(begin(parsedData), end(parsedData),
        [](auto& lhs, auto& rhs) { return lhs->getDate() < rhs->getDate(); });
