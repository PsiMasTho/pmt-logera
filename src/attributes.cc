...
{
    m_attrs.push_back(pair(name, vector<regex_match_checker>{}));
}
...
bool attributes::valid_value(size_t idx, string const& value) const
{
...
size_t attributes::get_count() const
{
    // assuming m_names.size() == m_checkValueRegexFuncs.size()
    return m_attrs.size();
}

size_t attributes::get_idx(std::string const& name) const
{
...
string const& attributes::get_name(size_t idx) const
{
