...
// ast's. The returned ast's are a MULTIFILE_NODE, containing one or more 'FILE_NODE's.
// which contain the decl nodes. The decl nodes are sorted by name within each file node.
void pass_2(
    ast_node* node
,   ast_node* out_decl_attrs
,   ast_node* out_decl_vars
,   opaque_vector* errors
);

// This pass merges all duplicate 'DECL_ATTR_NODE's and 'DECL_VAR_NODE's into one.
